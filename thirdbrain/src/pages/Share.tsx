...
import { DataResponse, Content } from "../types/types"; // Import global types
    ...
    <div className="flex items-center justify-center h-screen bg-sky-50">
      <div>
        {!data || !data.content ? (
          ...
          <>
            <h1 className="text-2xl font-bold mb-5 tracking-wider">{data.username}</h1>
            <div className="flex gap-5 flex-wrap">
              {data.content.map((con: Content) => (
                <Card
                  key={con._id}
                  title={con.title}
                  link={con.link}
                  ...
                  contentId={con._id}
                  tags={con.tags} // Passing `tags` to Card
                />
              ))}
            </div>
          </>
